...
export async function processOrders(
    ...
    while (true) {
        ...
        try {
                                    ...
                                    if (fleet.state.MoveSubwarp.arrivalTime - currentUnixTimestamp < randomWithinRange(-6, 0)) {
                                        {
                                            ...
                                            if (Ix.length > 0) {
                                                try {
                                                    stopWarps.push(executeStopWarp(Ix, walletSigner, connection));
                                                }
                                                catch (err) {
                                                    myLog(err.message);
                                                }
                        ...
                        case 'SDU': {
                            if (orders[x].auto) {
                                ...
                                if (fleet.state.Idle && fleet.state.Idle.sector[0] == orders[x].baseSector[0] && fleet.state.Idle.sector[1] == orders[x].baseSector[1] &&
                                    ...
                                    if (Ix.length > 0) {
                                        try {
                                            sduScans.push(executeScan(Ix, walletSigner, connectionSecondary, x, fleet, trackerBefore));
                                        }
                                        catch (err) {
                                            myLog(err.message);
                                            orders[x].refreshData = true;
            ...
            await Promise.all(stopWarps);
            await Promise.all(sduScans);
        }
        catch (err) {
            myLog(err.message);
        }
...
async function executeStopWarp(
    instructions: InstructionReturn[],
    signer: AsyncSigner,
    connection: Connection
): Promise<boolean> {
...
function awaitRateLimit() {
    while (new Date().getTime() - lastTxSend < 100)
        sleep(60);
